...
class MultiplexEnvelopeGameNetwork:
    ...
    def __init__(self, N, G, T, L, p, a, cl, ch, b, d1, d2, lmbd, beta, mu=0, strategy=0):
        """
        Constructor for the simulation
        :param N: number of nodes
        ...
        :param strategy: 0 - node plays in one layer in each iteration, 1 - node plays with a neighbor from each layer,
                         2 - node chooses random neighbor from random layer and plays with him in all layers
        :return:
        """
        # Check if the constraints are satisfied
        self.check_parameters(L, d1, d2, lmbd, beta, strategy)
        # initialize parameters
        self.N = N
        self.G = G
        self.T =  T
    ...
    def check_parameters(self, L, d1_vec, d2_vec, lmbd_vec, beta_vec, strategy):
        """
        Check whether the parameters satisfy the envelope game constraints
        :return:
        """
        ...
        \nThe following errors have been found:"""
        error = False
        if strategy != 0 and strategy != 1 and strategy != 2:
            constraints_desc += "\nPlease choose a valid strategy from [0, 1, 2]"
            error = True
    ...
    def perform_simulation(self):
        ...
        for t in range(self.T):
            ...
            if self.strategy == 0:
                ...
                layer = np.random.randint(0, self.L)
                opponents = [np.random.choice(np.where(self.G[layer][:, player_i] == 1)[0])]
                for i in range(self.L):
                    self.perform_simulation_in_layer(role, player_i, i, opponents)



    def perform_simulation_in_layer(self, role, player_i, layer, opponents = None):
        """
        Perform the simulation in the given layer between the player and his opponents
        :param role: the role of the player
        :param player_i: the id of the player
        :param layer: the layer in which the game will be played
        :param opponents: a list of opponents, if not specified the player will play against the neighbors in the layer
        :return:
        """
        payoff_i = 0
        if opponents == None:
    ...
    def get_players(self):
        """
        Getter for the player strategies in each layer
        :return: the player strategy list
        """
        return self.players